<x-livewire-tables::bs5.table.cell>
    <div class="d-inline-block align-top custom-space">
        <a class="text-primary-800 text-decoration-none"
           data-id="{{ $row->id }}">{{ $row->full_name }}</a>
        <span class="d-block text-muted fw-bold h-0">{{ $row->email ? $row->email : 'N/A' }}</span>
    </div>
</x-livewire-tables::bs5.table.cell>
...
<x-livewire-tables::bs5.table.cell>
    <a data-bs-toggle="tooltip"
       ...
       href="javascript:void(0)" class="btn px-2 text-info fs-3 pe-0 py-2 user-transaction-show-btn"
       data-id="{{ $row->id}}">
