...
<div class="col-12 col-lg-4">
    <div class="main-sidebar">
        <div class="single-sidebar-widget author-box-widegts radius-four">
            <div class="wid-title">
                <h3>About Me</h3>
            </div>
            <div class="author-info text-center">
                <div class="author-img object-fit-cover" {{$style}}="
                background-image: url('{{ !empty($news->user->profile) ? $news->user->profile : asset('front_landing/web/media/avatars/150-26.jpg')}}');background-size: 100% 100%;" alt=""></div>
            <h4>{{$news->user->first_name.' '.$news->user->last_name}}</h4>
        </div>
    </div>
        ...
        <div class="widget_categories">
            <ul>
                @foreach($newsCategories as $newsCategory)
                    <li><a href="{{ route('landing.news' , ['category' => $newsCategory->id])}}"
                           data-id="{{$newsCategory->id}}"
                           class="news-category-filter news-category-filter1 radius-four">{{isset($newsCategory) ? $newsCategory->name : ''}}
        ...
        <div class="tagcloud">
            @foreach($newsTags as $newsTag)
                @if(count($newsTag->news) > 0)
                    <a href="{{ route('landing.news' , ['tag' => $newsTag->id])}}" class="news-tags-filter radius-four"
